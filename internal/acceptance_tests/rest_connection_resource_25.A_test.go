...
func TestAccSaviyntRESTConnectionResource25A(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create
			{
				Config: testAccRESTConnectionResourceConfig25A(filePath, "create"),
			...
			// Update
			{
				Config: testAccRESTConnectionResourceConfig25A(filePath, "update"),
				ConfigStateChecks: []statecheck.StateCheck{
			...
			{
				Config:      testAccRESTConnectionResourceConfig25A(filePath, "update_connection_name"),
...
func testAccRESTConnectionResourceConfig25A(jsonPath, operation string) string {
	return fmt.Sprintf(`
provider "saviynt" {
  server_url = "%s"
  username   = "%s"
