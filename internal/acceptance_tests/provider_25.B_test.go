...
// TestAccProtoV6ProviderFactoriesWithEcho25B is used to instantiate a provider during acceptance testing.
// The factory function is called for each Terraform CLI command to create a provider
// server that the CLI can connect to and interact with.
var TestAccProtoV6ProviderFactories25B = map[string]func() (tfprotov6.ProviderServer, error){
	"saviynt": providerserver.NewProtocol6WithError(provider.New("test")()),
}

// TestAccProtoV6ProviderFactoriesWithEcho25B includes the echo provider alongside the scaffolding provider.
// It allows for testing assertions on data returned by an ephemeral resource during Open.
// The echoprovider is used to arrange tests by echoing ephemeral data into the Terraform state.
// This lets the data be referenced in test assertions with state checks.
var TestAccProtoV6ProviderFactoriesWithEcho25B = map[string]func() (tfprotov6.ProviderServer, error){
	"saviynt": providerserver.NewProtocol6WithError(provider.New("test")()),
	"echo":    echoprovider.NewProviderServer(),
}

func TestAccPreCheck25B(t *testing.T) {
	requiredEnvVars := []string{
		"SAVIYNT_URL_25B",
		"SAVIYNT_USERNAME_25B",
		"SAVIYNT_PASSWORD_25B",
	}
