...
func TestAccSaviyntDBConnectionDataSource25B(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDBConnectionDataSourceConfig25B(filePath),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(datasource, "connection_attributes.entitlement_exist_json", createCfg["entitlement_exist_json"]),
...
func testAccDBConnectionDataSourceConfig25B(jsonPath string) string {
	return fmt.Sprintf(`
provider "saviynt" {
  server_url = "%s"
  username   = "%s"
