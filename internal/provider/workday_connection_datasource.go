...
func (d *workdayConnectionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err != nil {
		if finalHttpResp != nil && finalHttpResp.StatusCode != 200 {
			log.Printf("[ERROR] HTTP error while reading Workday Connector: %s", finalHttpResp.Status)
			...
			resp.Diagnostics.AddError(
				"HTTP Error",
				fmt.Sprintf("HTTP error while reading Workday Connector for the reasons: %s", fetchResp["msg"]),
			)
