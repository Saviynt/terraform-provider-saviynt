...
func TestAccSaviyntRESTConnectionDataSource25B(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRESTConnectionDataSourceConfig25B(filePath),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(datasource, "connection_attributes.application_discovery_json", createCfg["application_discovery_json"]),
					resource.TestCheckResourceAttr(datasource, "connection_attributes.create_entitlement_json", createCfg["create_entitlement_json"]),
					resource.TestCheckResourceAttr(datasource, "connection_attributes.delete_entitlement_json", createCfg["delete_entitlement_json"]),
					resource.TestCheckResourceAttr(datasource, "connection_attributes.update_entitlement_json", createCfg["update_entitlement_json"]),
				),
			},
		},
	})
...
func testAccRESTConnectionDataSourceConfig25B(jsonPath string) string {
	return fmt.Sprintf(`
provider "saviynt" {
  server_url = "%s"
  username   = "%s"
