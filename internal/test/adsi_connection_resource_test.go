...
// adsi_connection_resource_test.go contains the Terraform acceptance test suite for
// the `saviynt_adsi_connection_resource`. It validates the resource’s full lifecycle against
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Import
			{
				...
				ImportStateVerifyIgnore: []string{"msg", "error_code", "password", "username"},
			},
			// Update
			{
...
func testAccADSIConnectionResourceConfig(jsonPath, operation string) string {
	return fmt.Sprintf(`
provider "saviynt" {
  server_url = "%s"
  username   = "%s"
