...
// entra_id_connection_resource_test.go contains the Terraform acceptance test suite for
// the `saviynt_entraid_connection_resource`. It validates the resource’s full lifecycle against
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Import
			{
				...
				ImportStateVerifyIgnore: []string{"msg", "error_code", "client_secret", "windows_connector_json", "connection_json", "client_id"},
			},
			// Update
			{
...
func testAccEntraIdConnectionResourceConfig(jsonPath, operation string) string {
	return fmt.Sprintf(`
provider "saviynt" {
  server_url = "%s"
  username   = "%s"
